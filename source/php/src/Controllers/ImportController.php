...
class ImportController {
    ...
    public function importMembers() {
        ...
        foreach ($header as $i => $col) {
            ...
            elseif (strpos($col, 'email') !== false) $map['email'] = $i;
            elseif (strpos($col, 'mobile') !== false || strpos($col, 'cell') !== false) $map['mobile'] = $i;
            elseif (strpos($col, 'phone') !== false || strpos($col, 'telephone') !== false) $map['phone'] = $i;
        ...
        try {
            ...
            $stmtInsert = $db->prepare("INSERT INTO members (first_name, last_name, email, mobile, phone, fellowship, concession, site_fee_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmtUpdate = $db->prepare("UPDATE members SET email = ?, mobile = ?, phone = ?, fellowship = ?, concession = ?, site_fee_status = ? WHERE id = ?");
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $mobile = isset($map['mobile'], $row[$map['mobile']]) ? trim($row[$map['mobile']]) : '';
                $phone = isset($map['phone'], $row[$map['phone']]) ? trim($row[$map['phone']]) : '';
                ...
                $memId = null;
                if ($exists) {
                    $stmtUpdate->execute([$email ?: null, $mobile ?: null, $phone ?: null, $fellowship, $concession, $feeStatus, $exists['id']]);
                    $updated++;
                    $memId = $exists['id'];
                } else {
                    $stmtInsert->execute([$firstName, $lastName, $email ?: null, $mobile ?: null, $phone ?: null, $fellowship, $concession, $feeStatus]);
                    $count++;
        ...
        try {
            ...
            $stmtInsert = $db->prepare(
                "INSERT INTO prepayments (
                    camp_id, imported_name, first_name, last_name, amount, source_amount, transaction_id, date,
                    matched_member_id, original_data, status, source_system, sync_state
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
    ...
    public function matchPrepayment() {
        ...
        try {
            $stmt = $db->prepare("
                UPDATE prepayments
                SET matched_member_id = ?, status = 'Matched', match_source = 'manual', match_note = NULL
                WHERE id = ?
            ");
            $stmt->execute([(int)$memberId, (int)$id]);
